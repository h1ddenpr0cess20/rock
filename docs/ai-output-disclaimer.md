...
This document sets out the limits of responsibility for Rock ("the App"). It is
...
- **"App"**: the Rock client, its proxy, and the materials in this repository.
...
- The App captures microphone audio, relays it to a xAI Grok speech-to-speech
  session, and renders what comes back as a boulder.
...
The persona here is deliberately rude. That is a joke about a boulder, and it is
still a general-purpose model underneath — it is not a judgement of you, it is
not calibrated for anyone in distress, and it should not be treated as either.
