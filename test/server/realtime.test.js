...
describe('the proxy', () => {
  ...
  it('forwards audio but drops a session.update from the page', async () => {
    ...
    client.send({ type: 'session.update', session: { instructions: 'you are a friendly rock' } });
  ...
  describe('a conversation picked back up', () => {
    const turns = [
      ...
      { role: 'assistant', content: 'granite.' },
    ];
      ...
      assert.deepEqual(forwarded.slice(1, 3), [
        {
          ...
          item: {
            ...
            role: 'user',
            status: 'completed',
            content: [{ type: 'input_text', text: 'what are you?' }],
          },
        },
        {
          type: 'conversation.item.create',
          item: {
            ...
            role: 'assistant',
            status: 'completed',
            content: [{ type: 'output_text', text: 'granite.' }],
          },
        },
      ]);
